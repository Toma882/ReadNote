...
public class ProtoEditor : EditorWindow
{
    ...
    private void OnEditGUI()
    {
        ...
        for (int i = 0; i < messages.Count; i++)
        {
            ...
            foldoutDic[message] = EditorGUILayout.Foldout(
                foldoutDic[message], message.name, true);
            //插入新类
            ...
            {
                //编辑类名
                message.name = EditorGUILayout.TextField(
                    "Name", message.name);
                //字段数量为0 提供按钮创建
                ...
                else
                {
                    for (int j = 0; j < message.fieldsList.Count; j++)
                    {
                        ...
                        GUILayout.BeginHorizontal();
                        //修饰符类型
                        item.modifier = (ModifierType)EditorGUILayout
                            .EnumPopup(item.modifier);
                        //字段类型
                        item.type = (FieldsType)EditorGUILayout
                            .EnumPopup(item.type);
                        if (item.type == FieldsType.Custom)
                        {
                            item.typeName = GUILayout
                                .TextField(item.typeName);
                        }
                        //编辑字段名
                        item.name = EditorGUILayout.TextField(item.name);
                        GUILayout.Label("=", GUILayout.Width(15f));
                        //分配标识号
                        item.flag = EditorGUILayout.IntField(
                            item.flag, GUILayout.Width(50f));
                        //插入新字段
                        if (GUILayout.Button("+", GUILayout.Width(20f)))
                        {
                            message.fieldsList.Insert(j + 1, 
                                new Fields(message.fieldsList.Count + 1));
                            Repaint();
                            return;
                        }
                        //删除该字段
    ...
    private void OnBottomMenuGUI()
    {
        ...
        if (GUILayout.Button("Export Json File"))
        {
            ...
            {
                //文件夹路径
                string dirPath = string.Format("{0}{1}/json",
                    Application.dataPath, workspacePath);
                //文件夹不存在则创建
        ...
        {
            //选择json文件路径
            string filePath = EditorUtility.OpenFilePanel("Import Json File", 
                string.Format("{0}{1}/json",
                    Application.dataPath, workspacePath), "json");
            //判断路径有效性
        ...
        if (GUILayout.Button("Generate Proto File"))
        {
            ...
            {
                string protoFilePath = EditorUtility.SaveFilePanel(
                    "Generate Proto File", new DirectoryInfo(Application.dataPath)
                        .Parent.FullName + "/protogen/Proto", fileName, "proto");
                        ...
                        for (int n = 0; n < message.fieldsList.Count; n++)
                        {
                            ...
                            switch (field.type)
                            {
                                ...
                                FieldsType.Custom: 
                                    ...
                                    break;
                                default:
                                    sb.Append(field.type.ToString().ToLower());
                                    break;
                            }
                            //空格
                            sb.Append(" ");
        ...
        if (GUILayout.Button("Create .bat"))
        {
            ...
            string rootPath = EditorUtility.OpenFolderPanel(
                "Create .bat file（protogen.exe所在的文件夹）", 
                new DirectoryInfo(Application.dataPath).Parent.FullName 
                    + "/protogen", string.Empty);
            //取消
            ...
            else
            {
                ...
                for (int i = 0; i < protos.Length; i++)
                {
                    ...
                    //拼接编译指令
                    sb.Append(rootPath + @"/protogen.exe -i:Proto\" 
                        + proto + @" -o:cs\" 
                        + proto.Replace(".proto", ".cs") + "\r\n");
                }
                sb.Append("pause");
...
public class Fields
{
    ...
    public bool IsValid()
    {
        return type != FieldsType.Custom
            || (type == FieldsType.Custom 
