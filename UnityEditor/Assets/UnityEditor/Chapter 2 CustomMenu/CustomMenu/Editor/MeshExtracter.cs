...
public class MeshExtracter
{
    ...
    public static void Execute()
    {
        ...
        try
        {
            ...
            AssetDatabase.CreateAsset(instance,string.Format("Assets/{0}.asset", mesh.name)); //创建资产 
            AssetDatabase.Refresh(); //刷新
            Selection.activeObject = instance; //选中
        }
        catch (Exception error)
        {
            Debug.Log(string.Format("{0}提取Mesh网格失败: {1}",Selection.activeGameObject.name, error));
        }
    }
