...
- [Unity 面试题总结 - 性能优化](#unity-面试题总结---性能优化)
  ...
  - [24. 简述UI资源如何优化](#24-简述ui资源如何优化)
    - [纹理资源优化](#纹理资源优化)
    - [UI纹理图集](#ui纹理图集)
  - [25. Unity JobSystem 多线程优化](#25-unity-jobsystem-多线程优化)
    - [什么是 JobSystem？](#什么是-jobsystem)
    - [核心接口](#核心接口)
      - [1. IJob - 单任务](#1-ijob---单任务)
      - [2. IJobParallelFor - 并行循环](#2-ijobparallelfor---并行循环)
      - [3. 依赖关系管理](#3-依赖关系管理)
    - [NativeContainer 类型](#nativecontainer-类型)
    - [实际应用场景](#实际应用场景)
      - [场景1：大量顶点计算](#场景1大量顶点计算)
      - [场景2：大量数学计算](#场景2大量数学计算)
      - [场景3：物理计算](#场景3物理计算)
    - [注意事项](#注意事项)
      - [1. 内存分配器](#1-内存分配器)
      - [2. 主线程同步](#2-主线程同步)
...
## 25. Unity JobSystem 多线程优化

### 什么是 JobSystem？

Unity JobSystem 是一个**多线程并行计算框架**，不仅仅是"工作线程管理器"，它包含：

1. **工作线程池管理** - 自动管理线程池，根据CPU核心数分配任务
2. **任务调度系统** - 依赖关系管理、任务优先级调度、自动负载均衡
3. **内存安全机制** - NativeContainer（NativeArray、NativeList等）保证线程安全
4. **并行计算接口** - IJob、IJobParallelFor、IJobParallelForTransform
...
#### 2. IJobParallelFor - 并行循环

```csharp
struct ParallelCalculateJob : IJobParallelFor
{
...
void Update()
{
    ...
    ParallelCalculateJob job = new ParallelCalculateJob
    {
        positions = positions,
    ...
    // 并行执行，自动分配到多个工作线程
    JobHandle handle = job.Schedule(positions.Length, 64); // 64是批次大小
...
```csharp
struct Job1 : IJob { public void Execute() { /* ... */ } }
struct Job2 : IJob { public void Execute() { /* ... */ } }
struct Job3 : IJob { public void Execute() { /* ... */ } }
...
| 类型 | 说明 | 使用场景 |
|------|------|----------|
| **NativeArray<T>** | 固定大小的数组 | 大量数据并行处理 |
| **NativeList<T>** | 动态大小的列表 | 需要动态添加数据 |
| **NativeHashMap<K,V>** | 哈希表 | 键值对查找 |
| **NativeQueue<T>** | 队列 | 生产者-消费者模式 |
...
struct UpdateVerticesJob : IJobParallelFor
{
    ...
    public void Execute(int index)
    {
        ...
        // 波浪效果计算
        vertex.y = Mathf.Sin(vertex.x * waveSpeed + time) * 0.5f;
...
void Update()
{
    if (meshVertices.IsCreated)
    {
        ...
        job.Schedule(meshVertices.Length, 64).Complete();
...
```csharp
// Allocator.Temp - 最快，但只能存活1帧
NativeArray<int> temp = new NativeArray<int>(100, Allocator.Temp);

// Allocator.TempJob - 工作线程使用，需要手动释放
NativeArray<int> job = new NativeArray<int>(100, Allocator.TempJob);
job.Dispose(); // 必须释放！

// Allocator.Persistent - 长期存在，性能最差
...
```csharp
// ❌ 错误：在工作线程中访问Unity API
struct BadJob : IJob
{
    public void Execute()
    {
        Debug.Log("错误！"); // Unity API只能在主线程调用
        GameObject.Find("Player"); // 错误！
    }
}
...
{
    [ReadOnly] public NativeArray<Vector3> input;  // 只读，安全
    public NativeArray<float> output;              // 写入，需要保证每个index只写一次
    
    public void Execute(int index)
    {
        // IJobParallelFor 保证每个index只执行一次，所以安全
...
|------|-------------------|
| **主线程循环** | ~2.5ms |
| **JobSystem (IJob)** | ~0.8ms |
| **JobSystem (IJobParallelFor)** | ~0.3ms |
...
**标准回答**：
> Unity JobSystem 是一个多线程并行计算框架，通过 `IJob`、`IJobParallelFor` 等接口，可以将CPU密集型计算分配到工作线程执行。使用 `NativeContainer`（如 `NativeArray`）保证线程安全，通过 `JobHandle` 管理依赖关系。适合大量数学计算、网格处理等场景。

**加分回答**：
> 我们项目在性能优化时考虑过 JobSystem，但最终选择了架构层面的优化（如 DataHandleQueue 批量处理、GridSystem 位图优化）。JobSystem 适合 CPU 密集型计算，而我们的瓶颈更多在逻辑层和渲染层。对于需要大量并行计算的场景（如寻路、物理模拟），JobSystem 能显著提升性能。
