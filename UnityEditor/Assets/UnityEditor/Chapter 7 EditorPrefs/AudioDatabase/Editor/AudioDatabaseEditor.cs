...
public class AudioDatabaseEditor : Editor
{
    ...
    private void OnEnable()
    {
        ...
        //EditorApplication.update 是 Unity 编辑器中的一个事件，用于在编辑器中执行更新操作。
    ...
    public override void OnInspectorGUI()
    {
        ...
        if (GUILayout.Button("Create New Audio Group"))
        {
            ...
            audioGroup.id = database.groups.Count == 0 ? 100 : (database.groups.Count + 100);
            AssetDatabase.AddObjectToAsset(audioGroup, database);
            database.groups.Add(audioGroup);
            AssetDatabase.ImportAsset(AssetDatabase.GetAssetPath(audioGroup));
        }
    ...
    private void OnAudioGroupGUI(AudioGroup group)
    {
        ...
        {
            if (EditorUtility.DisplayDialog("提醒", "是否确认删除该音频数据组", "确认", "取消"))
            {
        ...
        GUILayout.BeginHorizontal();
        {
            ...
            if (containsMouse)
            {
                switch (Event.current.type)
                {
                    ...
                    case EventType.DragPerform:
                        ...
                        {
                            int index = group.datas.FindIndex(m => m.clip == audioClip);
                            if (index == -1)
                            {
                                Undo.RecordObject(database, "Add Audio Data");
                                int newDataId = group.datas.Count == 0 ? 1000 : (group.datas.Count + 1000);
            ...
            GUI.color = new Color(GUI.color.r, GUI.color.g, GUI.color.b, containsMouse ? .9f : 0.5f);
    ...
    private void OnAudioDataGUI(AudioGroup group, AudioData data)
    {
        ...
        // Audio Data ID
        int newDataId = EditorGUILayout.IntField(data.id, GUILayout.Width(60f));
        ...
        // 若该音频正在播放 计算其播放进度 
        string progress = players.ContainsKey(data) ? ToTimeFormat(players[data].time) : "00:00:000";
        GUI.color = new Color(GUI.color.r, GUI.color.g, GUI.color.b, players.ContainsKey(data) ? 0.9f : 0.5f);

        // 显示信息：播放进度 / 音频时长 (00:00 / 00:00)
        GUILayout.Label(string.Format("({0} / {1})", progress, data.clip != null ? ToTimeFormat(data.clip.length) : "00:00:000"), progressStyle, GUILayout.Width(100f));
    ...
    private string ToTimeFormat(float time)
    {
        ...
        return string.Format("{0:D2}:{1:D2}:{2:D3}", minutes, seconds, millSecounds);
    }
}